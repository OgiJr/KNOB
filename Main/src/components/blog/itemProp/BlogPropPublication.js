...
import BlogClassicData from "../../../data/blog/BlogPublications.json";
import BlogListArchived from "./BlogListArchived";
import { FaSpinner } from "react-icons/fa";
import filters from "./Filters";

const alldataBLog = BlogClassicData;
const BlogPropPublication = ({ column, StyleVarProp, archive = "false" }) => {
        ...
        <div className="row row--15">
          {visibleItems.map((item) => (
            <div key={item.id} className={column}>
              <BlogListArchived StyleVar={StyleVarProp} data={item} archive={archive} type="publication" />
            </div>
          ))}
        </div>
      </div>
...
export default BlogPropPublication;
