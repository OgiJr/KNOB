...
const DashboardPublicationsKpe = () => {
  const { data } = useSWR(`${process.env.REACT_APP_API_URL}/api/get-ethics-protocols`, fetcher);
      ...
      <Modal scroll width="600px" open={visibleAdd} onClose={() => setVisibleAdd(false)}>
        <Form
          onSubmit={async (e) => {
            ...
            const resp = await fetch(`${process.env.REACT_APP_API_URL}/api/post-ethics-protocol`, {
              method: "POST",
              body: body,
              headers: {
                Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).token}`,
        ...
        <Form>
          ...
          <Modal.Footer>
            <Button auto onClick={async () => {
              ...
              const new_res = await fetch(`${process.env.REACT_APP_API_URL}/api/delete-ethics-protocol`, {
                method: "DELETE",
                body: new_body,
                headers: {
                  Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).token}`,
