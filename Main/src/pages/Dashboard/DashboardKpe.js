      ...
      <Modal scroll open={visibleAdd} onClose={() => setVisibleAdd(false)}>
        <Form
          onSubmit={async (e) => {
            ...
            const resp = await fetch(`${process.env.REACT_APP_API_URL}/api/post-ethics-committee-member`, {
              method: "POST",
              body: body,
              headers: {
                Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).token}`,
        ...
        <Form>
          ...
          <Modal.Footer>
            <Button auto color="error" onClick={async () => {
              ...
              const new_res = await fetch(`${process.env.REACT_APP_API_URL}/api/delete-ethics-committee-member`, {
                method: "DELETE",
                body: new_body,
                headers: {
                  Authorization: `Bearer ${JSON.parse(localStorage.getItem("user")).token}`,
